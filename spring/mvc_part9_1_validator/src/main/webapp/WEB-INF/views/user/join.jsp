	...
	<form id="joinForm" action="${path}/user/joinPost" method="post">
		<table class="container table table-bordered maxWidth">
			...
			<tr>
				...
				<td>
					<div class="row">
						...
						<div class="col-md-4">
							<input type="button" class="form-control btn btn-default" onclick="simple6_execDaumPostcode();" value="주소찾기"/>
						</div>
					</div>
					<br/>
			...
			<tr>				
				<td colspan="2">
				<h1>이용약관</h1>
				<textarea class="form-control" readonly>당신의 개인정보는 언제든지 회사에서 사용할수 있으며...10원에 팔아먹을수도 있어도.... 그래도 이 사이트를 이용하시겠습니까??</textarea>
				<label>
					<input type="checkbox" name="u_info" id="u_info" vlaue="y"/>
					개인정보 이용동의
				</label>
			</tr>
			<tr>
...
<script>
	function simple6_execDaumPostcode(){
		new daum.Postcode({
			oncomplete : function(data){
				...
				// 선택한 주소 타입에 따라 주소갑을 가져온다.
				if(data.userSelectType === 'R'){//도로명 주소
				...
				if(data.userSelectType === 'R'){
					...
					if(data.buildingName !== ''){
						...
						extraAddr += (extarAddr !== '' ?','+ data.buildingName : data.buildingName);
					}
	...
	$(function(){
		...
		// 메세지를 보여줄 요소, 보여줄 error message, 성공/실패 여부
		function showErrorMessage(elP, messageP, isChecked){
			// true <sapn style="margin-left:5px;font-size:12px:coloer:green;>사용 가능합니다.</span>"
			// false <sapn style="margin-left:5px;font-size:12px:coloer:red;>messageP</span>"
