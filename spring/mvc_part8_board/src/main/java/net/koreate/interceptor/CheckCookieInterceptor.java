...
public class CheckCookieInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		 ...
		 * i<cookies.length; i++) { if(cookies[i].getName().equals("signInCookie")) {
		 * 
		 * String uid = cookies[i].getValue(); UserVO uv = service.getUserById(uid);
		 * if(uv != null) { session.setAttribute("userInfo", uv); } } } }
		 */
