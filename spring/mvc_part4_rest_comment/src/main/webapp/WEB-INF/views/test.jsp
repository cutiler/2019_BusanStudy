	...
	<script>
		...
		function getAllList() {
			$.getJSON("comments/all/" + bno, function(data) {
					...
					$(data).each(function() {
						str += "<li data-cno='"+this.cno+"' data-text='";
						str += this.commentText+"' class='commentLi'>"+ this.cno+ " : "+ this.commentText;
						str += "&nbsp;<button>MODIFY</button></li>";
					});
					$("#comments").html(str);
			});
		}
		...
		function printPage(pageMaker) {
			var str = "";
			if (pageMaker.prev) {
				str += "<li "+strClass+"><a href='"+(pageMaker.startPage-1)+"'> << </a></li>";
			}
			...
			if (pageMaker.next) {
				str += "<li "+strClass+"><a href='"+(pageMaker.endPage+1)+"'> >> </a></li>";
			}
			$("#pagination").html(str);			
		}
